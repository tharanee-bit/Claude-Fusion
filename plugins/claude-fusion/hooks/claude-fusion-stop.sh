...
[ -n "$DIFF_FULL" ] || { clf_dbg "empty diff -> exit"; rm -f "$MARKER" 2>/dev/null; clf_cleanup_subagent_turn "$STATE_KEY"; exit 0; }
...
[ -n "$CLF_OUTPUT" ] || { record_review_failure; clf_dbg "empty review -> exit (marker kept)"; exit 0; }
...
[ -n "$REVIEW" ] || { record_review_failure; clf_dbg "review extraction failed (marker kept)"; exit 0; }
